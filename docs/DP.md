...
Without a parameter file, only an explicit ε is accepted: `setup_client_context` derives σ from `dp_epsilon` and `dp_delta`, and the sentinel value 10.0 raises instead of silently running with a weak budget. Parameter files are JSON; legacy pickle files are refused.

The `dp_params.json` file on disk is **not modified**. The override exists only in memory for the duration of the process. Re-running with the sentinel value 10.0 uses the original key file $\varepsilon$ again.
...
Concurrent sweep experiments (if run in parallel, e.g., in a CI environment) are safe because they access the parameter file in read-only mode and write to disjoint output directories.
