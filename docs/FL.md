...
The **ZKP modes** bound each client's update: a client proves with a Groth16 zk-SNARK that ‖w_local − w_global‖₂ is within a bound the server sets, and the server rejects clients whose proofs fail. In `zkp` and `he_elgamal_zkp` every coordinate is covered and the proof is bound to what the server aggregates; the sampled modes prove server-sampled coordinates after a commitment, so a violation is caught with the probability given in [ZKP.md, section 6.4](ZKP.md#64-he_elgamal_zkp_sampled). Soundness rests on the Groth16 assumptions over BN254 and on a trusted setup performed by a single party ([ZKP.md, section 7.3](ZKP.md#73-what-a-single-party-setup-does-and-does-not-give)).
...
**The ZKP bound does not guarantee gradient correctness.** It limits how far one admitted client can move the global model per round, not in which direction: a malicious update within the bound is admitted. In the sampled modes, coordinates that are not sampled carry no proof. See [ZKP.md, section 13](ZKP.md#13-limitations).
...
**HE + ZKP composition.** In `he_tenseal_zkp` and `he_concrete_tfhe_zkp` (and their DP variants) the proof covers a plaintext vector the client chooses and is not bound to the ciphertext the server aggregates, so a client can prove one update and upload the encryption of another: these modes provide confidentiality only. `he_elgamal_zkp` binds the proof to the aggregated ciphertexts and to the encrypted global model ([ZKP.md, section 6](ZKP.md#6-modes-and-protocols)).
