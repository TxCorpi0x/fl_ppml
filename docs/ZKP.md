...
In this framework, the SRS is generated by a single party with `gnark_service setup`, once per circuit, and the verifying keys are pinned by hash in `zkp_gnark_service/keys/manifest.json` (audit/setup.md). This is acceptable for research and experimental purposes — the threat model does not include adversaries attempting to forge ZKP proofs by compromising the trusted setup. For a production healthcare FL deployment, a proper ceremony should be conducted. This is acknowledged as a known limitation in the framework's security documentation.
...
The gnark proof service runs as two HTTP processes. The prover loads the pinned proving keys and compiles each fixed-size circuit at startup. The verifier loads only the pinned verifying keys. Neither runs setup: keys come from a separate `gnark_service setup` step, and a process whose keys don't match the manifest refuses to start. Inputs shorter than a circuit's fixed size are padded (zeros for the norm circuit, encryptions of zero for ElGamal). The verifying key is a small object, a few KB, whose size grows only with the number of public inputs. This is architecturally clean: the gnark service is a cryptographic co-processor, and the Python FL framework calls it via HTTP without managing any Go or cryptographic code directly.
...
| Parameter | Value |
|-----------|-------|
| Coordinates per proof | fixed by `zkp_gnark_service/keys/manifest.json`; shorter chunks are padded |
| R1CS constraints, key sizes | recorded per circuit in the manifest and audit/setup.md |
| Trusted setup | Groth16 **per-circuit**, single-party (`gnark_service setup`), BN254 curve |
...
| Problem | Cause | Fix |
|---------|-------|-----|
| `Connection refused :9000`/`:9001` | gnark prover/verifier not started | see README "gnark ZKP Service" |
...
| Service refuses to start / HTTP 503 on a key mismatch | keys differ from the pinned manifest | Regenerate with `gnark_service setup --force` and commit `zkp_gnark_service/keys/` (this re-pins the key) |
