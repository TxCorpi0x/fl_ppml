...
---------------------------
Whether the proofs *verified*. The ZKP aggregation paths currently re-admit
every client when none verify (audit/findings.md S1-02), and the anchor is
built from that re-admitted set, so a fail-open round and a verified round
leave identical ledger entries. Certifying verification outcomes requires the
fail-closed work in Step 4.
"""
...
VERIFICATION_NOT_RECORDED = (
    "proof verification outcomes are not persisted; a fail-open round is "
    "indistinguishable from a verified one (audit/findings.md S1-02)"
)
