"""
fl.keys.concrete_tfhe — Concrete TFHE key management.

Pre-compiled key bundles live in ``fl/keys/prebuilt/`` (formerly
``fl/concrete_tfhe_keys/``).  Each bundle is named::
...
:func:`fl.core.security.get_concrete_aggregation_context` → ``ctx.generate_keys()``.
This module provides helpers for that flow and for inspecting prebuilt bundles.

Usage::

    from fl.keys.concrete_tfhe import generate, list_prebuilt, prebuilt_dir
...
# Alias for unified fl.keys dispatcher
load = generate
