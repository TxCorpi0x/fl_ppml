...
2048-bit safe-prime group and exposes helpers used by ppflx/privacy/zkp.py.
...
WHAT IS MISSING — why the pedersen backend provides no Byzantine protection:
  ...
  3. Serialisation and wire transport: commitments and proofs would need to
     ...
     them.  Currently ppflx/privacy/zkp.py discards all commitment objects and
...
def read_zkp_params(file_path: str) -> ZKPContext:
    ...
    try:
        ...
        raise ValueError(
            ...
            "because unpickling can execute code). Regenerate: python -m ppflx.keys generate zkp"
        ) from exc
