"""
ppflx.keys.cli — Unified CLI for cryptographic key/parameter management.

Usage::

    python -m ppflx.keys generate he_tenseal [--secret SECRET] [--public PUBLIC] [--overwrite]
    python -m ppflx.keys generate dp         [--output OUTPUT] [--epsilon E] [--delta D] ...
    python -m ppflx.keys generate zkp        [--output OUTPUT] [--bit_length N] [--overwrite]
    python -m ppflx.keys generate he_concrete_tfhe [--bit_width N] [--num_clients N] ...
    python -m ppflx.keys generate he_elgamal [--overwrite]
    python -m ppflx.keys list
    python -m ppflx.keys prebuilt            # list Concrete TFHE prebuilt bundles
"""
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "list":
        from ppflx.keys import list_modes
    ...
    if args.command == "prebuilt":
        from ppflx.keys.concrete_tfhe import list_prebuilt, prebuilt_dir
    ...
    try:
        if mode == "he_tenseal":
            ...
            from ppflx.keys.zkp import generate
            ...
            from ppflx.keys.concrete_tfhe import generate
