...
class DifferentialPrivacyMode(PrivacyMode):
    ...
    def setup_client_context(self, config) -> Any:
        ...
        """
        from ppflx.core.security import DifferentialPrivacyParams, load_dp_params
        ...
        # it silently weakened the privacy budget.
        if getattr(config, "dp_epsilon", 10.0) == 10.0:
            raise FileNotFoundError(
                f"DP params not found: {dp_path}\n"
                "Run: python -m ppflx.keys generate dp, or pass an explicit --dp_epsilon."
            )
        import numpy as _np
