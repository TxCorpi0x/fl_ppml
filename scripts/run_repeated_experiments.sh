...
if [[ ! -f tenseal_context.bin && ! -f keys/tenseal_context.bin ]]; then
...
if [[ ! -f dp_params.json && ! -f keys/dp_params.json ]]; then
    echo "[PREREQ] DP params not found. Run: python -m fl.keys generate dp --overwrite"
    PREREQ_FAIL=1
fi
