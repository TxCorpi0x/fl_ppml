...
if [[ ! -f keys/he_tenseal/secret_key.pkl ]]; then
...
if [[ ! -f keys/dp/dp_params.pkl ]]; then
    echo "[PREREQ] DP params not found. Run: python -m fl.keys generate dp --output keys/dp/dp_params.pkl --overwrite"
    PREREQ_FAIL=1
fi
